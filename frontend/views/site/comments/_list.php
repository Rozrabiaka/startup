    ...
    <div class="comment-content" data-comment-content-id="<?php echo $model->id; ?>">
        <div class="comment-details">
            ...
            <div class="comment-author-name">
				...
				<?php echo Html::a('Опубліковано ' . $model->getPostedDate(), $model->getAnchorUrl(), ['class' => 'comment-date']); ?>
            </div>
        </div>
        ...
        <div class="comment-action-buttons">
			...
			<?php if (!Yii::$app->user->isGuest && ($model->level < $maxLevel || is_null($maxLevel))) : ?>
				<?php echo Html::a("<span class='glyphicon glyphicon-share-alt'></span> " . Yii::t('yii2mod.comments', 'Відповісти'), '#', ['class' => 'reply-comment-btn', 'data' => ['action' => 'reply', 'comment-id' => $model->id]]); ?>
			<?php endif; ?>
        </div>
    </div>
</li>
